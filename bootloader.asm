...
mov bx, KERNEL_OFFSET ; Address where to load the data
mov cl, 2             ; Start after this bootloader
...
jmp CODE_SEGMENT_32:enter_protected_mode

; al: Number of sectors to read
; bx: Address where to load the data
; cl: First sector to read
; dl: Drive number
read_from_disk:
pusha         ; Save the current state
push ax       ; Save the number of sectors to read
mov ah, 2     ; Read mode
mov ch, 0     ; Use the first cylinder
mov dh, 0     ; Use the first head
int 0x13
jc disk_error_1

pop dx        ; Restore the number of sectors to read
cmp al, dl    ; Verify the specified number of sectors were read
jne disk_error_2

popa          ; Restore the saved state and return
ret

disk_error_1:
mov si, DISK_ERROR_MESSAGE_1
call bios_print
jmp $

disk_error_2:
mov si, DISK_ERROR_MESSAGE_2
call bios_print
jmp $
...
a20_error:
mov byte [abs 0xB8000], 0x68
mov byte [abs 0xB8001], 15
jmp $
...
INTERRUPT_DESCRIPTOR_TABLE_OFFSET equ 0x80000
...
; Error messages
DISK_ERROR_MESSAGE_1 db 'D1', 0
DISK_ERROR_MESSAGE_2 db 'D2', 0
